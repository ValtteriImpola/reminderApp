...
//                }
                ...
                setMapLongClick(map = map, navController, viewState,viewModel)
    ...
    map.setOnMapLongClickListener { latlng ->
        ...
        reminders.forEach {reminder ->
            val markerOptions = MarkerOptions()
                .title(reminder.message)
                .position(LatLng(reminder.location_x, reminder.location_y))
            val distance = SphericalUtil.computeDistanceBetween(latlng, LatLng(reminder.location_x, reminder.location_y))
            if (distance < 1000){
                map.addMarker(markerOptions)
            }
        ...
        map.setOnMarkerClickListener (GoogleMap.OnMarkerClickListener() {
            viewModel.textToSpeech(it.title)
            false
        })
//        map.addMarker (
